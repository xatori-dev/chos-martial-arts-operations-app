...
describe("post-login operations app", () => {
  ...
  it("opens a rebuilt student Profile page with reference layout for student accounts", () => {
    ...
    expect(within(feedPanel).getByText("4 Messages")).toBeInTheDocument();
  ...
  it("toggles the student bottom panel between belt case and messages without resetting message search", () => {
    ...
    expect(screen.queryByRole("region", { name: "Belt case customize panel" })).not.toBeInTheDocument();
    expect(screen.getByLabelText("Student reference profile card")).toBeInTheDocument();
    expect(within(bottomPanel).getByText("4 Messages")).toBeInTheDocument();
  ...
  it("lets managers publish study guide folders while student study routes return to Profile", async () => {
    ...
    expect(screen.getByLabelText("Student profile page")).toBeInTheDocument();
    expect(screen.getByLabelText("Student reference belt journey")).toBeInTheDocument();
    expect(screen.queryByLabelText("Student app launcher")).not.toBeInTheDocument();
    expect(screen.queryByLabelText("Student study guide materials")).not.toBeInTheDocument();
  });

  it("lets managers upload categorized videos while student video routes return to Profile", async () => {
    ...
    expect(screen.getByLabelText("Student reference belt journey")).toBeInTheDocument();
    expect(screen.queryByLabelText("Student app launcher")).not.toBeInTheDocument();
    expect(screen.queryByLabelText("Student video library")).not.toBeInTheDocument();
  });
  ...
  it("redirects student accounts away from the removed Student's Panel", () => {
    ...
    expect(screen.queryByLabelText("Manager app launcher")).not.toBeInTheDocument();
    expect(screen.queryByLabelText("Manager navigation")).not.toBeInTheDocument();
    expect(screen.queryByRole("heading", { name: "Student's Panel" })).not.toBeInTheDocument();
